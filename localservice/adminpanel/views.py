...
from .models import AdminUser,  Review
...
def dashboard(request):
    counts = {
        ...
        'approved_providers': Provider.objects(is_approved=True).count(),
        'pending_providers': Provider.objects(is_approved=False).count(),
...
def list_providers(request):
    ...
    return render(request, 'adminpanel/list_providers.html', {'providers': providers})
...
def approve_provider(request, provider_id):
    ...
    if not Service.objects(provider=provider):
        Service(
...
def delete_provider(request, provider_id):
    ...
    if provider:
        Service.objects(provider=provider).delete()  # Delete all services
        provider.delete()
    return redirect('adminpanel:providers')
